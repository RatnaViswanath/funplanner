...
async def run_planner_agent(
    ...
    # ── Agentic loop: max 20 rounds to allow 3-plan generation ──────────────────
    for round_num in range(20):
        ...
        # After round 12, inject a strong nudge to produce the final JSON
        if round_num >= 12:
            messages.append({
                "role": "user",
                "content": "You have gathered enough data. STOP calling tools now. Produce the final JSON with all 3 plans immediately. No more tool calls."
            })
